...
function draw() {
    ...
    surface2.Draw();
    gl.bindTexture(gl.TEXTURE_2D, textureRegular);
    gl.clear(gl.DEPTH_BUFFER_BIT);
    const audioPos = vecFromAngles(alpha, beta, gamma)
    modelViewProjection = m4.translation(0.5 * audioPos[0], 0, 0)
    if (panner) {
    ...
    if (deviceOrientation) {
        ...
        modelViewProjection = m4.multiply(strCam.ProjectionMatrix, m4.multiply(strCam.ModelViewMatrix, m4.multiply(matAccum1, deviceOrientation)));
    }
    gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false, modelViewProjection);
...
function initGL() {
    ...
    textureRegular = LoadTexture();
    CreateWebCamTexture()

    strCam = new StereoCamera(10, 1, 1, 120, 1, 10);
    ...
    surface2.BufferData(surface2Vertices(), surface2Vertices(), surface2Textures());
...
function getRotationMatrixFromAngleVector(rotationVector) {
...
function gyroscopeToRotationMatrix() { // should be run every draw call
    if (x !== null) {
        let dT = (Date.now() - timestamp) * MS2S;

        let omegaMagnitude = Math.sqrt(x * x
            + y * y + z * z);
        ...
        if (omegaMagnitude > E) {
            ...
            alpha = Math.min(Math.max(alpha, -Math.PI * 0.3), Math.PI * 0.3)
            beta = Math.min(Math.max(beta, -Math.PI * 0.3), Math.PI * 0.3)
            gamma = Math.min(Math.max(gamma, -Math.PI * 0.3), Math.PI * 0.3)
            let angleVector = [];
            angleVector.push(alpha);
            angleVector.push(beta);
            angleVector.push(gamma);

            tangibleMatrix = getRotationMatrixFromAngleVector(angleVector)
            document.getElementById('message').innerText = tangibleMatrix
            timestamp = Date.now();
...
let timestamp,
    ...
    beta = 0,
    gamma = 0;
let deviceOrientation = m4.identity();
function init() {
    initAudio();
    timestamp = Date.now();
